...
x_hat = x_hat[:,0]
coeff_unravel = grates.utilities.unravel_coefficients(x_hat, max_degree=95)
...
plt.hist(spaceGrid.value_array.flatten(), bins=32, density=True)
plt.ylabel("frequency")
plt.xlabel("Joule/kg")
plt.xlim(-0.2,0.2)
plt.grid()

mu, std = norm.fit(spaceGrid.value_array.flatten())
print(mu,std)
x = np.linspace(-0.2,0.2, 100)
p = norm.pdf(x, mu, std)
plt.plot(x, p, 'k', linewidth=2, label=r'$\mathcal{N}(0.007,0.037)$')
plt.legend()
plt.savefig('hist.png', dpi=300)

plt.figure(2)
plt.hist(spaceGrid_kaula10000.value_array.flatten(), bins=32, color='orange', density=True)
plt.ylabel("frequency")
plt.xlabel("Joule/kg")
plt.xlim(-0.2,0.2)
plt.grid()
